        ...
        <div className="rounded-2xl border border-slate-800 bg-slate-900/80 p-5">
          ...
          <div className="space-y-3">
            {filteredUsers.slice(0, 50).map((account) => {
              ...
              return <div key={account.id} className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-slate-800 bg-slate-950/60 p-3"><div><p className="font-medium text-white">{account.whatsapp}</p><p className="text-sm text-slate-400">{account.subdomain} • {proxy?.label || 'Unknown proxy'}</p></div><div className="flex items-center gap-2"><span className={`rounded-full px-3 py-1 text-xs ${account.status !== 'active' ? 'bg-slate-700/70 text-slate-300' : soon ? 'bg-amber-500/15 text-amber-300' : 'bg-emerald-500/15 text-emerald-300'}`}>{account.status !== 'active' ? account.status : soon ? 'Soon to expire' : account.expires_at}</span><Link to="/proxies" className="rounded-lg border border-slate-700 px-3 py-2 text-xs text-cyan-300">Manage</Link></div></div>;
            })}
